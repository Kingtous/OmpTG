...
root = '/home/rtco/EFG/'
# =======DOT存放位置===============
dotPath = root + 'fib_sweet.dot'
# =======relation.txt存放位置======
relationPath = root + 'relation.txt'
# =======需要处理的函数入口======
parseFunction = 'fib'
# =======WCET目录====================
wctPath = root + 'fib.wct'
# ===========cluster_定义==========
Definition = ''
# ========输出================================
# =======dot输出==========
dotOutput = root + 'fib_pro.dot'
# =======特征输出==========
Edges = 0
Nodes = 0
Call_TaskFunc = 0
...
def NodeWait(graph):
    ...
    graph.add_edge('_taskFunc1__exit', 'concom__bb7__2', color='green')
    graph.add_edge('_taskFunc0__exit', 'concom__bb7__2', color='green')
...
def connectEdgeForSubgraph(EFG, relationDict):
...
def genEFG(graph, path, keylist, relation):
    ...
    connectEdgeForSubgraph(EFG, relation)
    # 解决图的重复问题
    ...
    # 输出DOT文件
    nx.nx_pydot.write_dot(EFG, str(output_cnt) + '.dot')
    file = open(str(output_cnt) + '.dot', 'r')
    fileData = file.readlines()
    file.close()
    delFile(str(output_cnt) + '.dot')
    file = open(str(output_cnt) + '_EFG.dot', 'w')
    ...
    file.close()
    # 将DOT转换输出PDF文件
    os.system('dot -Tpdf ' + str(output_cnt) + '_EFG.dot' + ' -o ' + str(output_cnt) + '_EFG.pdf')
...
def genFuntionRelativity(graph, relation):
    '''
    :param graph: 经过前面处理过的CFG图
    :param relation: statement与函数调用的图
    :return: 返回funtion依赖的函数字典
    '''
    for callblock in function_set:
        try:
            # TODO 查找边，用networkx会导致找不出循环
    ...
    return getSubgraphForFunction(graph, keylist, parseFunction)


def getSubgraphForFunction(graph, keylist, parseFunction):
    subgraph = nx.MultiDiGraph()
    # 先计算当前path需要加入那些函数path
    functionsForCurrent = set()
    functionsForCurrentQueue = queue.Queue()
    # 入口点开始，开始广度优先搜索需要添加的结点
    functionsForCurrentQueue.put(parseFunction)
    while not functionsForCurrentQueue.empty():
        functionLabel = functionsForCurrentQueue.get()
        if functionLabel not in keylist:
            # Maybe System Library
            continue
        elif functionLabel in functionsForCurrent:
            continue
        for vertex in ModeDict[functionLabel].nodesList:
            if vertex in relation.keys() and not relation[vertex].startswith('ort_'):
                functionsForCurrentQueue.put(relation[vertex])
        functionsForCurrent.add(functionLabel)
    for item in functionsForCurrent:
        subgraph = nx.compose(subgraph, nx.subgraph(graph, ModeDict[item].nodesList))
    # 预留生成图的代码，以便后续需要
    connectEdgeForSubgraph(subgraph, relation)

    outputDict = {}
    for item in functionsForCurrent:
        outputDict[item] = ModeDict[item].nodesList
...
if __name__ == '__main__':
    ...
    print("提取 " + parseFunction + " 结点的subgraph...")
    # parseFunction　在第20行修改
    print(genFuntionRelativity(graph, relation))
